...
<template>
  <div class="tmsgBox" ref="tmsgBox">
    <div class="tmsg-respond" ref="respondBox">
      <h3>发表评论</h3>
      <form class="">
        <el-input type="textarea" :rows="2" placeholder="说点什么呢``" v-model="textarea" :disabled="isComment === '1'">
        </el-input>
        ...
        <el-row class="tmsg-r-info">
          <el-col :span="24" class="info-submit">
            <!--                        <p class="tcolors-bg" @click="sendMsg" :style="visible()">{{sendTip}}</p>-->
            <el-button v-show="isRespond" class="tcolorm" @click="removeRespond">取消回复</el-button>
            <el-button type="primary" @click="sendMsg" style="width: 200px"
              :disabled="isComment === '1'">{{ sendTip }}</el-button>
          </el-col>
        </el-row>
      </form>
    </div>
          ...
          <li class="tmsg-c-item" v-for="(item, index) in commentList" :key="'common' + index">
            <article class="">
              ...
              <section>
                ...
                <div v-if="haslogin" class="tmsg-replay" @click="respondMsg(item.id, item.id, item.createdBy)">
                  回复
                </div>
              </section>
            </article>
              ...
              <li class="tmsg-c-item" v-for="(citem, cindex) in item.children" :key="'citem' + cindex">
                <article class="">
                  <header>
                    ...
                    <div class="i-name">
                      <span style="color: powderblue;">{{ citem.username }}</span> <span>回复</span>
                      {{ citem.toCommentUserName }}
                    </div>
                    <div class="i-time">
                      <time>{{ citem.createdTime }}</time>
                    </div>
                  </header>
                  <section>
                    <p v-html="analyzeEmoji(citem.content)"></p>
                    <div v-show="haslogin" class="tmsg-replay" @click="respondMsg(item.id, citem.id, citem.createdBy)">
                      回复
                    </div>
                  </section>
                </article>
...
import { getStudioComment } from "@/api/comment";
...
export default {
  ...
  data() { //选项 / 数据
    return {
      ...
      haslogin: false,
      userId: '',//用户id
      type: this.commentType,//回复评论的当前的commentId
      ...
      sendTip: '发送~',
      /*      OwOlist:[//表情包和表情路径
              {'title':'微笑','url':'/assets/img/emot/image/emote/weixiao.gif'},
  ...
  methods: {
    visible() {
      return this.isComment === '0' ? 1 : 0
    },
    //事件处理器
    setData(initData, result) {
      console.log(result.records)
      var msg = result.records;
      console.log("--------msg------")
      console.log(msg)
      console.log("--------total-----")
    ...
    analyzeEmoji: function (cont) {//编译表情替换成图片展示出来
      ...
      if (content) {
        for (var i = 0; i < content.length; i++) {
          ...
          // str = str.replace(pattern2,'<img src='+src+'"../../../static/img/emot/image/"/>');
          str = str.replace(pattern2, '<img src="static/img/emot/image/" />');
...
/*      if (getToken() == null || getToken() == '') {
        this.$message.error("当前会话已过期请从新登录！")
        return
      }*/

      if (that.textarea) {
        that.sendTip = '咻~~';
        sendComment(that.type, that.sid, that.rootId, that.toCommentId, that.toCommentUserId, that.textarea).then((response) => {
          that.textarea = '';
          that.rootId = -1;
          that.toCommentId = -1;
    ...
    showCommentList: function (initData) {//评论列表
      var that = this
      that.queryParams.studioId = this.sId;
      //判断当前用户是否登录
      ...
      //公用设置数据方法
      if (that.$route.name == 'details') {//文章列表的评论
        //that.type = 0;
        getStudioComment(that.queryParams).then((response) => {
          console.log("----response------")
          console.log(response)
          that.setData(initData, response);
        })
      } else {//其他评论
        if (that.$route.name == 'photographer') {
         // that.type = 1
          getStudioComment(that.queryParams).then((response) => {
            that.setData(initData, response);
          })
